...
class ModflowDataset(Dataset):
    ...
    stage / tau select the time window. The two stages solve on disjoint
    windows -- stage 1 on [tmin, tau], stage 2 on (tau, tmax] -- and each
    normalises t onto [-1, 1] using its OWN window, so a snapshot from the
    wrong stage is not merely useless: it is evaluated far outside the range
    the network is fitted on, and it inflates the hard-constraint factor d by
    the same ratio. This filter is carried over verbatim from the benchmark-1
    trainer; the benchmark-2 copy of this file had dropped it.

    Coordinates are returned in MODEL coords (0..1000) exactly as written by
    MODFLOW. The shift to the network's centred coords (-500..500) is applied
    in Trainer._next_data_batch, which is where benchmark 1 does it.
    """
    def __init__(self, pattern="./modflow/sdata/t*.txt", stage=1, tau=1.0,
                 device=None, verbose=True):

        if verbose:
            print("\n--- MODFLOW Dataset ---")
            print(f"stage={stage}, tau={tau}, pattern={pattern}")
        rows_xyt = []
        rows_h   = []
        self.times = []
        ...
        if not rows_xyt:
            raise ValueError(
                f"No MODFLOW rows loaded for {pattern!r} with stage={stage}, "
                f"tau={tau}. Check that sdata/ exists and covers this stage's "
                f"time window.")
        ...
        if verbose:
            ...
            print(f"kept {len(self.times)} snapshots, t = {t.min():g} .. {t.max():g}")
            print(f"{self.X.shape[0]:,} points | "
                  f"x {self.X[:,0].min():.1f}..{self.X[:,0].max():.1f} "
                  f"y {self.X[:,1].min():.1f}..{self.X[:,1].max():.1f} "
                  f"(MODEL coords; shifted at batch time) | "
                  f"h {self.H.min():.2f}..{self.H.max():.2f}")
            print("--- End Dataset ---\n")
