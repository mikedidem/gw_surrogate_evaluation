#!/usr/bin/env python
"""Benchmark B1: homogeneous conductivity, uniform boundary head.
...
B1 shares its implementation with B2 and differs only in two constants, so the
chronological split, leakage audit and solver guards carry over unaltered:

  1. K_lo = K_hi, so the low-conductivity lens disappears. K_of() returns a
     constant 33.33 m/d with grad K identically zero, and the Boussinesq term
     h*(grad K . grad h) vanishes exactly rather than numerically.

  2. h_north = h_south = 90 m, so the Dupuit profile
     sqrt(h_s^2 + (h_n^2 - h_s^2) * frac) collapses to a constant. The initial
     condition and both Dirichlet boundaries are 90 m.
...
class Problem(object):
    def __init__(self,
        ...
        # ---- no regional gradient: both Dirichlet edges at 90 m ----
        self.h_north = 90.0
        self.h_south = 90.0

        # ---- homogeneous conductivity (K_lo == K_hi) ----
        self.K_hi = 33.33          # background, the published baseline value
        self.K_lo = 33.33          # equal to K_hi: no lens
    ...
    def __repr__(self):
        return (f'unconfined well problem, homogeneous K={self.K_hi} m/d, '
                f'uniform boundary head {self.h_south} m')
...
def well_center_from_field(path, shift=500.0):
    """Locate the well in network coordinates from a MODFLOW `x y h` export.

    Returns the position of the deepest drawdown cell, converted to the centred
    frame. For a single-well benchmark that cell is the pumping cell, so this
    recovers the value xi should hold. Use a late snapshot, where the cone is
    deepest and least ambiguous.
    """
    d = np.loadtxt(path)
    i = int(np.argmin(d[:, 2]))
    return (float(d[i, 0]) - shift, float(d[i, 1]) - shift)
