...
def plot_test_cnn_vs_modflow(
    ...
    x = np.unique(df0["x"].values) - shift_xy
    y = np.unique(df0["y"].values) - shift_xy
    ...
    for i, (mf_file, cnn_file) in enumerate(zip(modflow_files, cnn_files)):
        ...
        vmin = min(np.nanmin(grid_h_mf), np.nanmin(grid_h_cnn))
        vmax = max(np.nanmax(grid_h_mf), np.nanmax(grid_h_cnn))
